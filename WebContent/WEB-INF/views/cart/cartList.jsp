<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="cart.model.vo.Cart, java.util.ArrayList, java.text.DecimalFormat" %>
<%
...
/* 	input[type="button"] {height: 30px; background : white;	border : none; margin-top:10px;	margin-right:20px;} */
 	input[type="checkbox"] {margin: 15px; width: 13px; height: 13px; vertical-align: middle;}
	
	.layout {width:100%; padding-right: 15px; padding-left: 15px; margin-right: auto; margin-left: auto; max-width: 960px;}
	.layout2 {width:100%; max-width: 1300px; margin-right: auto; margin-left: auto;}
	#space4 {height: 30px;}
	#space8 {height: 70px;}
	#space9 {height: 50px; width: 960px; padding: 5px 5px 5px 0px;}
	#space10 {height: 60px;}
	#space11 {height: 66px;}
	#space12 {height: 230px;}
 	#cart_line {background: #C3C3C3; height: 2px; width: 960px;}
	#subject {font-weight: bold; height: 105px; text-align: center; font-size: 40px; padding: 20px;}
	#subject2 {font-weight: bold; width: 960px; height: 90px; font-size: 40px; padding: 20px;}
	
	#order_no {width: 960px; height: 90px; vertical-align: center; text-align: center;}
	...
	#total_price_list {border: 3px solid #E0E0E0; width: 960px; height: 211px; text-align: center;}
	.total {display: inline-block; text-align: center; vertical-align: middle;}
	#total_product_price {margin: auto 39px auto auto;}
	#total_price_bar {margin: 26px 39px 26px 39px; background-image: url("img/total_price_bar.png");
						background-attachment: fixed; width: 2px; height: 155px;}
	#total_shipping_price {margin: auto 39px auto 39px;}
	#total_payment_price {margin: auto auto auto 39px;}
	#tpp_name {font-size: 30px; color: #6CB174; font-weight: bold; text-align: center; margin-bottom: 33px;}
	#tpp_conts {font-size: 25px; text-align: center;}
	#tsp_name {font-size: 30px; color: #6CB174; font-weight: bold; text-align: center; margin-bottom: 33px;}
	#tsp_conts {font-size: 25px; text-align: center;}
	#tpmp_name {font-size: 30px; color: #3C7F44; font-weight: bold; text-align: center; margin-bottom: 33px;}
	...
	#one_order {width: 80px; height: 25px; background: #3c7f44; color: white; border: 1px solid #3c7f44; font-size: 13px;
				margin-bottom: 5px;}
	#one_delete {width: 80px; height: 25px; background: white; color: black; border: 1px solid black; font-size: 13px;
				}
	
	#table-line {width: 960px; height: 5px; background: #7F7F7F;}
	#list {margin-left: auto; margin-right: auto; height: 40px; width: 960px; text-align: center;}
	#product {text-align: left; padding-left: 20px;}
	#list tr {border-bottom: 2px solid #CFCFCF;}
	#list td {height: 90px;}
	#list th {height: 45px;}
	#list tr input[type=checkbox] {border: 1px solid #a4a4a4; height: 30px;}

	/* 하단 버튼 */
	#space2 {height: 105px; text-align: center;}
	#space3 {height: 70px; text-align: center;}
	
	.selectbtn {width: 960px; height: 66px; display: flex;	justify-content: center; margin-bottom: 100px;}
	...
	#order_selected {border: 1px solid #BEBEBE; background: #BEBEBE; color:white;}
	#order_selected:hover {border: 1px solid #3C7F44; background: #3C7F44;}
	
	#order_all {border: 1px solid #BEBEBE; background: #BEBEBE; color:white;}
	#order_all:hover {border: 1px solid #3C7F44; background: #3C7F44;}
	table input:focus {outline:none;}
 	table input:not([name=amount],[name=pbtn],[name=mbtn]) {border: 0px;}
	table input[name=total] {width: 100px; text-align:center;}
	table input[name=price] {width: 100px; text-align:center;}
	table input[name=prodName] {width: 200px; text-align:center;}
</style>
</head>
<body>
	...
	<% } else { %>
		<form action="<%=request.getContextPath() %>/order.me">
			...
			<table id="list">
				<tr>
					<th width="30"><input type="checkbox" name="all" id="all1" onclick="selectAll();"></th>
				...
				<% if(list.isEmpty()) { %>
					<tr><td colspan="7">등록된 장바구니 상품이 없습니다.</td></tr>
				<% } else { %>
					<%
						for(int i = 0; i < list.size(); i++) { 
					%>
						<tr>
							<td><input type="checkbox" name="one" id="one<%= list.get(i).getCartNo() %>" onclick="selectOne();"></td>
							<td>
								<div id="product_detail">
			    					<input type="text" name="prodName" value="<%= list.get(i).getProdName() %>" readonly>
							...
							<td>
								<div id="prod_price" style="display: inline-block;">
									<input type="text" name="price" id="price<%= i %>" value="<%= list.get(i).getPrice() %>" readonly>
								</div>
							</td>
							<td>
				        		<input type="button" name="pbtn" id="minus<%= i %>" onclick="count('minus');" value="─"><!--
				        		 --><input hidden="hidden"><input type="text" id="amount<%= i %>" name="amount" value="1" oninput="return handleOnInput(this);"><!--
								 --><input type="button" name="mbtn" id="plus<%= i %>" onclick="count('plus');" value="┼">
								<input type="button" name="amountChange" id="amountChange<%= i %>" value="수량변경">
							</td>
							<td><div id="prod_sp" style="display: inline-block;">기본배송</div></td>
							<td>
								<div id="prod_sum" style="display: inline-block;">
									<input type="text" name="total" id="total<%= i %>" value="<%= list.get(i).getTotal() %>" readonly>
								</div>
							</td>
							<td>
								<div><input type="button" id="one_order" value="주문하기"></div>
								<div><input type="button" id="one_delete" value="삭제"></div>
							</td>
						</tr>
					<% } %>
				<% } %>
			...
			<div class="layout" id="space11">
				<div class="selectbtn">
					...
					<input type="button" id="order_selected" value="선택 상품 주문">
					<input type="submit" id="order_all" value="전체 상품 주문">
				</div>
			</div>
		</form>
