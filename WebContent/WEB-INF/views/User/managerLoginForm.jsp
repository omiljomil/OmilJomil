...
<script>
	function checkValue(){
        ...
        if(!form.managerCode.value){
            alert("관리자 인증번호를 입력하세요.");
            return false;
        }
        ...
        if(!isIdCheck){
    		...
    		alert("인증번호를 확인해주세요.");
    		return false;
    	}else if(!ismanagerCode){
    		alert("관리자 인증번호를 확인해주세요.");
    	}else{
	...
	$('#managerCode').on('chage paste keyup',function(){
		if($('#managerCode').val() == "5mil"){
			$('#codeNum').text("인증번호가 동일합니다.");
			$('#codeNum').css({'color':'green','float':'left','display':'inline-block'});
			ismanagerCode = true;
		}else{
			$('#codeNum').text("인증번호가 다릅니다.");
			$('#codeNum').css({'color':'red','float':'left','display':'inline-block'});
			ismanagerCode = false;
		}
